...
async function seed() {
  ...
  // 1. Seed Authentication accounts & User profiles
  for (const u of USERS) {
    try {
      ...
      try {
        ...
        uid = userRecord.uid;
        console.log(`Auth user already exists: ${u.email} (UID: ${uid})`);
      } catch (e) {
      ...
      // Write User profile to Firestore with correct UID
      ...
      console.log(`Seeded profile and capacities for ${u.email}`);
    } catch (e) {
  ...
  for (const o of ORG_UNITS) {
    ...
    console.log(`Seeded org unit: ${o.name}`);
  }
  ...
  for (const g of ACTIVITY_GROUPS) {
    ...
    console.log(`Seeded activity group: ${g.name}`);
  }

  // 5. Seed Activities
  for (const a of ACTIVITIES) {
    await db.collection('activities').doc(a.id).set(a, { merge: true });
    console.log(`Seeded activity: ${a.name}`);
  }
